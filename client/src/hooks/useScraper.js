...
export function useScraper(showToast, onScraperComplete) {
  ...
  const [scrapeProgress, setScrapeProgress] = useState({ pagesScraped: 0, totalPages: 0, currentConfig: 0, totalConfigs: 0, currentConfigName: '', lastError: null });
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (!status.running) {
            clearInterval(pollRef.current);
            setScraping(false);
            setScrapeProgress({ pagesScraped: 0, totalPages: 0, currentConfig: 0, totalConfigs: 0, currentConfigName: '', lastError: null });
            if (status.lastError) {
              showToast?.(`Fehler: ${status.lastError}`, 'error');
            } else {
  ...
  const stopScraping = useCallback(async () => {
    try {
      ...
      // Let polling continue – it detects running=false from the server
      // and correctly resets the scraping state.
    } catch (e) {
      showToast?.(`Fehler: ${e.message}`, 'error');
    }
  }, [showToast]);
