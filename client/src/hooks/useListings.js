...
export function useListings(showToast) {
  ...
  // Refreshes stats from the server after every mutation – more reliable than manual deltas.
  ...
  const handleBlacklist = useCallback(async (id) => {
    // Define unblacklist first (for Undo)
    const doUndo = async () => {
    ...
    setListings((prev) => prev.map((l) => (
      l.id === id ? { ...l, is_blacklisted: 1, is_favorite: 0, blacklisted_at: new Date().toISOString() } : l
    )));
    showToast?.('Auf Blacklist gesetzt', 'info', {
      duration: 5000,
      action: { label: 'Rückgängig', onClick: doUndo },
    });
    await refreshStats();
