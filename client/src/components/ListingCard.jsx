...
const ListingCard = memo(function ListingCard({
  ...
  const upgradeUrl = (u) =>
    u ? u.replace('/thumbs/images/', '/images/').replace(/s-l\d+\./, 's-l1600.') : u;
  ...
  const goNext = useCallback(
    (e) => {
      e.stopPropagation();
      e.preventDefault();
      setImgIdx((i) => (i + 1) % Math.max(resolvedImages.length, 1));
    },
  ...
  const goPrev = useCallback(
    (e) => {
      ...
      e.preventDefault();
      setImgIdx(
        (i) => (i - 1 + Math.max(resolvedImages.length, 1)) % Math.max(resolvedImages.length, 1),
      );
    },
    [resolvedImages.length],
  );
  ...
  const handleBlacklist = (e) => {
    e.stopPropagation();
    e.preventDefault();
    isBlacklistView ? onUnblacklist?.(l.id) : onBlacklist?.(l.id);
  };
  const handleSeenToggle = (e) => {
    e.stopPropagation();
    e.preventDefault();
    onSeen(l.id);
  };
  ...
  return (
    <article
      className={`card ${isNew ? 'card--new' : ''} ${l.is_seen ? 'card--seen' : ''} ${l.is_blacklisted ? 'card--blacklisted' : ''}`}
    >
      <div className="card-img">
        ...
        {resolvedImages.length > 1 && currentImg && (
          <>
            <button className="carousel-btn carousel-btn--prev" onClick={goPrev}>
              <ChevronLeft />
            </button>
            <button className="carousel-btn carousel-btn--next" onClick={goNext}>
              <ChevronRight />
            </button>
          </>
        )}
        {resolvedImages.length > 1 && (
          <span className="carousel-counter">
            {imgIdx + 1}/{resolvedImages.length}
          </span>
        )}
        ...
        <div className="card-actions-overlay">
          ...
          <button
            className={`card-fav-btn ${l.is_favorite ? 'card-fav-btn--active' : ''}`}
            onClick={handleFav}
            title={
              l.is_favorite
                ? 'Favorit entfernen'
                : isBlacklistView
                  ? 'Als Favorit (von Blacklist entfernen)'
                  : 'Als Favorit'
            }
          >
          ...
          <button
            className={`card-blacklist-btn ${l.is_blacklisted ? 'card-blacklist-btn--active' : ''}`}
            onClick={handleBlacklist}
            title={isBlacklistView ? 'Von Blacklist entfernen' : 'Blacklisten'}
          >
            <BlacklistIcon />
          </button>
        </div>
        ...
        <div className="card-badge-stack">
          {providerLabel && (
            <span
              className="card-badge card-badge--provider"
              style={{
      ...
      <div className="card-footer">
        <div className="card-dates">
          ...
          {l.first_seen && (
            <span
              className="card-date card-date--scraped"
              title={new Date(l.first_seen).toLocaleString('de-DE')}
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
          ...
          {seenMultipleTimes && (
            <span
              className="card-date card-date--scraped"
              title={new Date(l.last_seen).toLocaleString('de-DE')}
            >
              <svg
                viewBox="0 0 24 24"
                fill="none"
        ...
        <div className="card-footer-actions">
          {onDetails && (
            <button className="card-detail-btn" onClick={handleDetails}>
              Details
            </button>
          )}
          <a
