...
import { inferListingTypeFromUrl } from '../utils/url.js';
...
function AgentCard({ config, providers, isSelected, stats, onSelect, onToggle, onEdit, onDelete, onScrapeConfig, onResetConfig, onClearFavorites, onClearBlacklist, scraping }) {
  ...
  const typeLabel = LISTING_TYPE_LABELS[config.listing_type] || config.listing_type;
  ...
  return (
    <div
      ...
      style={{
        '--agent-accent': colors.text,
        '--agent-accent-soft': colors.bg,
      }}
    >
      <div className="agent-card-header">
        <div className="agent-card-main">
          <div className="agent-card-title-row">
            ...
            <span className="agent-card-city" title={displayName}>{displayName}</span>
          </div>

          <div className="agent-card-meta">
            <span className="agent-card-provider" style={{
              background: PROVIDER_COLORS[config.provider]?.bg ?? '#f3f4f6',
              color: PROVIDER_COLORS[config.provider]?.text ?? '#374151',
              border: `1px solid ${PROVIDER_COLORS[config.provider]?.border ?? '#d1d5db'}`,
            }}>
              {providerLabel}
            </span>
            <span className="agent-card-type" style={{ background: colors.bg, color: colors.text }}>
              {typeLabel}
            </span>
          </div>
        </div>
      ...
      <div className="agent-card-actions" onClick={(e) => e.stopPropagation()}>
        <button className="agent-action" onClick={() => onScrapeConfig(config.id)} disabled={scraping || !config.enabled} title="Scrapen" aria-label="Scrapen">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" width="13" height="13">
            <path d="M1 4v6h6M23 20v-6h-6" />
            <path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15" />
          </svg>
        </button>
        <button className="agent-action" onClick={() => onEdit(config)} title="Bearbeiten" aria-label="Bearbeiten">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
            <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
            <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
          </svg>
        </button>
        <button className="agent-action" onClick={() => onResetConfig(config.id)} title="Listings dieses Agents löschen (Favoriten behalten)" aria-label="Agent-Listings zurücksetzen">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
            <path d="M16 3l5 5L8 21H3v-5L16 3z" />
            <path d="M15 7l2 2" />
          </svg>
        </button>
        <button className="agent-action" onClick={() => onClearFavorites(config.id)} title="Favoriten dieses Agenten löschen" aria-label="Favoriten dieses Agenten löschen">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
            <line x1="2" y1="2" x2="22" y2="22"/>
          </svg>
        </button>
        <button className="agent-action" onClick={() => onClearBlacklist(config.id)} title="Blacklist-Einträge dieses Agenten leeren" aria-label="Blacklist dieses Agenten leeren">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
            <circle cx="12" cy="12" r="10"/>
            <line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/>
          </svg>
        </button>
        <button className="agent-action agent-action--danger" onClick={() => onDelete(config.id)} title="Agent löschen" aria-label="Agent löschen">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
    ...
    <form className="agent-form" onSubmit={handleSubmit}>
      ...
      <div className="agent-form-field">
        ...
        <input
          ...
          required
        />
        <span className="agent-form-hint">Name des Suchagenten</span>
      </div>
        ...
        <div className="agent-form-detected">
          {inferred ? (
            <span className="agent-form-detected-ok">
              <span className="agent-form-provider-badge" style={{
                background: (PROVIDER_COLORS[effectiveProvider]?.bg ?? '#f3f4f6'),
                color:      (PROVIDER_COLORS[effectiveProvider]?.text ?? '#374151'),
                border:     `1px solid ${PROVIDER_COLORS[effectiveProvider]?.border ?? '#d1d5db'}`,
              }}>
                {PROVIDER_LABELS[effectiveProvider] || effectiveProvider}
              </span>
              <span className="agent-form-type-badge" style={{
                background: (LISTING_TYPE_COLORS[effectiveType]?.bg ?? '#f3f4f6'),
                color:      (LISTING_TYPE_COLORS[effectiveType]?.text ?? '#374151'),
              }}>
                {LISTING_TYPE_LABELS[effectiveType] || effectiveType}
              </span>
            </span>
          ) : (
            <span className="agent-form-detected-fallback">Anbieter &amp; Typ werden automatisch aus der URL erkannt.</span>
          )}
        </div>
      </div>
          ...
          {configs.map(c => {
            ...
            return (
              <button
                ...
                title={`${c.name} · ${LISTING_TYPE_LABELS[c.listing_type] || c.listing_type}`}
