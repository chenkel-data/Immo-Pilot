...
// DELETE /api/listings/clear-favorites/:configId  – resets favorites for a single agent
router.delete(
...
// DELETE /api/listings/clear-blacklist  – clears the entire blacklist
router.delete(
  '/clear-blacklist',
  asyncHandler(async (_req, res) => {
    clearAllBlacklist();
...
// DELETE /api/listings/clear-blacklist/:configId  – clears the blacklist for a single agent
router.delete(
...
// DELETE /api/listings/reset  – deletes all listings (favorites & blacklist are retained)
router.delete(
  '/reset',
  asyncHandler(async (_req, res) => {
    purgeListingsKeepPinned();
...
// DELETE /api/listings/reset/:configId  – deletes listings for a single search agent
router.delete(
  '/reset/:configId',
  asyncHandler(async (req, res) => {
...
async function fetchImagesForListing(listing) {
  ...
  try {
    ...
    const galleryImgs = [
      ...html.matchAll(/class="galleryimage-element[^"]*"[^>]*>[\s\S]*?<img[^>]+src="([^"]+)"/g),
    ];
    for (const m of galleryImgs) urls.add(m[1]);

    const jsonLdMatch = html.match(
      /<script type="application\/ld\+json">[\s\S]*?"image":\s*(\[[\s\S]+?\])/m,
    );
    if (jsonLdMatch) {
      try {
    ...
    if (urls.size === 0) {
      const fallback = [
        ...html.matchAll(/https:\/\/img\.kleinanzeigen\.de\/api\/v1\/prod-ads\/images\/[^"'\s]+/g),
      ];
