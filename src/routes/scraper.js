...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ok: true,
      message: `Scraping started (${configs.length} configuration${configs.length > 1 ? 's' : ''}).`,
    });
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    progress = {
      pagesScraped: 0,
      totalPages: cfg.max_pages || 10,
      currentConfig: 1,
      totalConfigs: 1,
      currentConfigName: cfg.name || cfg.city,
      lastError: null,
    };
    ...
    res.json({
      ok: true,
      message: `Scraping started: ${cfg.name || cfg.city} (${cfg.listing_type}).`,
    });
